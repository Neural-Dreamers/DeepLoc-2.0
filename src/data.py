...
def get_swissprot_df(clip_len):
    ...
    nes_exclude_list = ['Q7TPV4', 'P47973', 'P38398', 'P38861', 'Q16665', 'O15392', 'Q9Y8G3', 'O14746', 'P13350',
                        'Q06142']
...
def get_swissprot_ss_Xy(save_path, fold, clip_len):
    ...
    X_true_train, y_true_train = np.concatenate(
        (np.stack(annot_true_df["embeds"].to_numpy()), np.stack(annot_true_df["Target"].to_numpy())), axis=1), np.stack(
        annot_true_df["TargetSignal"].to_numpy())
    annot_pred_df = seq_df
    X_pred_target = np.stack(annot_true_df["preds"].to_numpy())  # > threshold_dict[f"{i}_multidct"]
    X_pred_train, y_pred_train = np.concatenate((np.stack(annot_pred_df["embeds"].to_numpy()), X_pred_target),
                                                axis=1), np.stack(annot_pred_df["TargetSignal"].to_numpy())
    ...
    X_test, y_test = np.concatenate((np.stack(annot_test_df["embeds"].to_numpy()), X_test_target), axis=1), np.stack(
        annot_test_df["TargetSignal"].to_numpy())
...
class EmbeddingsLocalizationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        return self.data_df["Sequence"][index], embedding, self.data_df["Target"][index], self.data_df["TargetAnnot"][
            index], self.data_df["ACC"][index]
